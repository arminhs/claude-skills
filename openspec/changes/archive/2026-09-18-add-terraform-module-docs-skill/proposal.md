...
- Scope: the skill targets undocumented modules and modules whose README is empty, minimal, or fully generated by terraform-docs. On those it applies the aws-ia layout observed in `terraform-aws-vpc` as the default. Modules with a substantial hand-maintained README are out of scope: the audit detects and reports them, and the skill limits itself to source descriptions there.
